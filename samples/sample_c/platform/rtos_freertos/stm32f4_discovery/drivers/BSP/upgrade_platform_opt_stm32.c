...
/* Private constants ---------------------------------------------------------*/
#define GDU_TEST_UPGRADE_FILE_INFO_STORE_ADDR      (APPLICATION_STORE_ADDRESS_END - 1023)
#define GDU_TEST_UPGRADE_REBOOT_KEY                0x11223344

/* Private types -------------------------------------------------------------*/
typedef struct {
    uint32_t upgradeRebootKey;
    T_GduUpgradeEndInfo upgradeEndInfo;
} T_GduTestUpgradeRebootParam;
...
{
    FLASH_If_Write(GDU_TEST_UPGRADE_FILE_INFO_STORE_ADDR, (uint8_t *) &s_upgradeFileInfo,
                   sizeof(T_GduUpgradeFileInfo));
...
{
    uint32_t result;
    T_GduUpgradeFileInfo *upgradeFileInfo = (T_GduUpgradeFileInfo *) (GDU_TEST_UPGRADE_FILE_INFO_STORE_ADDR);
...
{
    uint32_t result;
    T_GduTestUpgradeRebootParam upgradeRebootParam;

    upgradeRebootParam.upgradeRebootKey = GDU_TEST_UPGRADE_REBOOT_KEY;
    ...
    result = FLASH_If_Write(APPLICATION_PARAM_STORE_ADDRESS, (uint8_t *) &upgradeRebootParam,
                            sizeof(T_GduTestUpgradeRebootParam));
    if (result != FLASHIF_OK) {
        return GDU_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }
                                                              ...
                                                              T_GduUpgradeEndInfo *upgradeEndInfo)
{
    ...
    upgradeRebootParam = *(T_GduTestUpgradeRebootParam *) (APPLICATION_PARAM_STORE_ADDRESS);

    if (upgradeRebootParam.upgradeRebootKey == GDU_TEST_UPGRADE_REBOOT_KEY) {
