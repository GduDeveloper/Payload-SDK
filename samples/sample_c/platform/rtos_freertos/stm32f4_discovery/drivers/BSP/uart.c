...
#define UART6_READ_BUF_SIZE      1024
#define UART6_WRITE_BUF_SIZE     1024
/* Private macro -------------------------------------------------------------*/
...
//UART1 mutex
static T_GduMutexHandle s_uart1Mutex;
//USART1 handle
...
static T_GduMutexHandle s_uart2Mutex;
...
static T_GduMutexHandle s_uart3Mutex;
...
static T_GduMutexHandle s_uart6Mutex;
...
/**
 * @brief UART6 interrupt request handler fucntion.
 */
#ifdef USING_UART_PORT_6
...
void USART6_IRQHandler(void)
{
    ...
    if (__HAL_UART_GET_IT_SOURCE(&s_uart6Handle, UART_IT_RXNE) != RESET &&
        ...
        data = (uint8_t) ((uint16_t) (s_uart6Handle.Instance->DR & (uint16_t) 0x01FF) & (uint16_t) 0x00FF);
    ...
    if (__HAL_UART_GET_IT_SOURCE(&s_uart6Handle, UART_IT_TXE) != RESET &&
        ...
        if (RingBuf_Get(&s_uart6WriteRingBuffer, &data, 1)) {
            /* Transmit Data */
            s_uart6Handle.Instance->DR = ((uint16_t) data & (uint16_t) 0x01FF);
        } else {
...
{
    if (GDU_CONSOLE_UART_NUM == UART_NUM_1) {
        HAL_UART_Transmit(&s_uart1Handle, (uint8_t *) &ch, 1, 0xFFFF);
    } else if (GDU_CONSOLE_UART_NUM == UART_NUM_2) {
        HAL_UART_Transmit(&s_uart2Handle, (uint8_t *) &ch, 1, 0xFFFF);
    } else if (GDU_CONSOLE_UART_NUM == UART_NUM_3) {
        HAL_UART_Transmit(&s_uart3Handle, (uint8_t *) &ch, 1, 0xFFFF);
    } else if (GDU_CONSOLE_UART_NUM == UART_NUM_6) {
        HAL_UART_Transmit(&s_uart6Handle, (uint8_t *) &ch, 1, 0xFFFF);
    }
