...
static T_GduReturnCode GetAttitudeInformation(T_GduGimbalAttitudeInformation *attitudeInformation);
static T_GduReturnCode GetCalibrationState(T_GduGimbalCalibrationState *calibrationState);
static T_GduReturnCode GetRotationSpeed(T_GduAttitude3d *rotationSpeed);
static T_GduReturnCode GetJointAngle(T_GduAttitude3d *jointAngle);
static T_GduReturnCode StartCalibrate(void);
static T_GduReturnCode SetControllerSmoothFactor(uint8_t smoothingFactor, E_GduGimbalAxis axis);
static T_GduReturnCode SetPitchRangeExtensionEnabled(bool enabledFlag);
static T_GduReturnCode SetControllerMaxSpeedPercentage(uint8_t maxSpeedPercentage, E_GduGimbalAxis axis);
static T_GduReturnCode RestoreFactorySettings(void);
static T_GduReturnCode SetMode(E_GduGimbalMode mode);
static T_GduReturnCode Reset(E_GduGimbalResetMode mode);
static T_GduReturnCode FineTuneAngle(T_GduAttitude3d fineTuneAngle);
static T_GduReturnCode GduTest_GimbalAngleLegalization(T_GduAttitude3f *attitude, T_GduAttitude3d aircraftAttitude,
                                                       T_GduGimbalReachLimitFlag *reachLimitFlag);
static T_GduReturnCode
GduTest_GimbalCalculateSpeed(T_GduAttitude3d originalAttitude, T_GduAttitude3d targetAttitude, uint16_t actionTime,
                             T_GduAttitude3d *speed);
static void GduTest_GimbalSpeedLegalization(T_GduAttitude3d *speed);
//static T_GduReturnCode GduTest_GimbalCalculateGroundAttitudeBaseQuaternion(T_GduFcSubscriptionQuaternion quaternion,
//                                                                           T_GduAttitude3d *attitude);

/* Private variables ---------------------------------------------------------*/
static T_GduTaskHandle s_userGimbalThread;
static T_GduGimbalCommonHandler s_commonHandler = {0};
static T_GduGimbalSystemState s_systemState = {0};
static bool s_rotatingFlag = false;
static T_GduMutexHandle s_commonMutex = {0};

static T_GduGimbalAttitudeInformation s_attitudeInformation = {0}; // unit: 0.1 degree, ground coordination
static T_GduAttitude3f s_attitudeHighPrecision = {0}; // unit: 0.1 degree, ground coordination
static T_GduGimbalCalibrationState s_calibrationState = {0};
static T_GduAttitude3d s_targetAttitude = {0}; // unit: 0.1 degree, ground coordination
static T_GduAttitude3d s_speed = {0}; // unit: 0.1 degree/s, ground coordination
static T_GduAttitude3d s_aircraftAttitude = {0}; // unit: 0.1 degree, ground coordination
static T_GduAttitude3d s_lastAircraftAttitude = {0}; // unit: 0.1 degree, ground coordination
static E_TestGimbalControlType s_controlType = TEST_GIMBAL_CONTROL_TYPE_UNKNOWN;
static const T_GduAttitude3d s_jointAngleLimitMin = {-1200, -100, -1800}; // unit: 0.1 degree
static const T_GduAttitude3d s_jointAngleLimitMax = {300, 100, 1800}; // unit: 0.1 degree
static const T_GduAttitude3d s_eulerAngleLimitMin = {-900, -100, -1800}; // unit: 0.1 degree
static const T_GduAttitude3d s_eulerAngleLimitMax = {100, 100, 1800}; // unit: 0.1 degree
...
static const T_GduAttitude3d s_speedLimit = {1800, 1800, 1800}; // unit: 0.1 degree/s
static uint32_t s_calibrationStartTime = 0; // unit: ms
static T_GduMutexHandle s_attitudeMutex = NULL;
static T_GduMutexHandle s_calibrationMutex = NULL;
...
T_GduReturnCode GduTest_GimbalStartService(void)
{
    ...
    s_commonHandler.FineTuneAngle = FineTuneAngle;//姿态微调角度

    if (osalHandler->MutexCreate(&s_commonMutex) != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("mutex create error");
        return GDU_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }

    if (osalHandler->MutexCreate(&s_attitudeMutex) != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("mutex create error");
        return GDU_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }

    if (osalHandler->MutexCreate(&s_calibrationMutex) != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("mutex create error");
        return GDU_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }
...
T_GduReturnCode GduTest_GimbalDeInit(void)
{
    ...
    gduStat = osalHandler->TaskDestroy(s_userGimbalThread);
    if (gduStat != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Destroy test gimbal thread error: 0x%08llX.", gduStat);
        return gduStat;
    }

    gduStat = GduGimbal_DeInit();
    if (gduStat != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Deinit gimbal module error: 0x%08llX.", gduStat);
        return gduStat;
    }

    if (osalHandler->MutexDestroy(s_calibrationMutex) != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("mutex destroy error");
        return GDU_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }

    if (osalHandler->MutexDestroy(s_attitudeMutex) != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("mutex destroy error");
        return GDU_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }

    if (osalHandler->MutexDestroy(s_commonMutex) != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("mutex destroy error");
        return GDU_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }
...
static void *UserGimbal_Task(void *arg)
{
    ...
    T_GduAttitude3f nextAttitude = {0};
    T_GduAttitude3f attitudeFTemp = {0};
    ...
    while (1) {
        ...
        if (USER_UTIL_IS_WORK_TURN(step, 50, PAYLOAD_GIMBAL_TASK_FREQ)) {
            ...
            gduStat = GduTest_GimbalCalculateGroundAttitudeBaseQuaternion(quaternion, &s_aircraftAttitude);
            if (gduStat != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        ...
        switch (s_systemState.gimbalMode) {
            case GDU_GIMBAL_MODE_FREE:
                break;
            case GDU_GIMBAL_MODE_FPV:
                ...
                break;
            case GDU_GIMBAL_MODE_YAW_FOLLOW:
                s_attitudeInformation.attitude.yaw += (s_aircraftAttitude.yaw - s_lastAircraftAttitude.yaw);
        ...
        GduTest_GimbalAngleLegalization(&attitudeFTemp, s_aircraftAttitude, &s_attitudeInformation.reachLimitFlag);
        ...
        GduTest_GimbalAngleLegalization(&s_attitudeHighPrecision, s_aircraftAttitude, NULL);

        attitudeFTemp.pitch = s_targetAttitude.pitch;
        attitudeFTemp.roll = s_targetAttitude.roll;
        attitudeFTemp.yaw = s_targetAttitude.yaw;
        GduTest_GimbalAngleLegalization(&attitudeFTemp, s_aircraftAttitude, NULL);
...
out2:
        ...
        gduStat = osalHandler->GetTimeMs(&currentTime);
        if (gduStat != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("get current time error: 0x%08llX.", gduStat);
            continue;
        }

        if (osalHandler->MutexLock(s_calibrationMutex) != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("mutex lock error");
            continue;
        }
        ...
        if (progressTemp >= 100) {
            ...
            s_calibrationState.currentCalibrationStage = GDU_GIMBAL_CALIBRATION_STAGE_COMPLETE;
        }

unlockCalibrationMutex:
        if (osalHandler->MutexUnlock(s_calibrationMutex) != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("mutex unlock error");
            continue;
        }
    }
...
static T_GduReturnCode StartCalibrate(void)
{
    ...
    gduStat = osalHandler->GetTimeMs(&s_calibrationStartTime);
    if (gduStat != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("get start time error: 0x%08llX.", gduStat);
    }
    ...
    s_calibrationState.currentCalibrationStage = GDU_GIMBAL_CALIBRATION_STAGE_PROCRESSING;
...
static T_GduReturnCode SetControllerSmoothFactor(uint8_t smoothingFactor, E_GduGimbalAxis axis)
{
    ...
    if (axis == GDU_GIMBAL_AXIS_PITCH)
        s_systemState.smoothFactor.pitch = smoothingFactor;
    else if (axis == GDU_GIMBAL_AXIS_YAW)
...
static T_GduReturnCode RestoreFactorySettings(void)
{
    ...
    s_systemState.pitchRangeExtensionEnabledFlag = false;
    s_systemState.gimbalMode = GDU_GIMBAL_MODE_FREE;
...
static T_GduReturnCode Reset(E_GduGimbalResetMode mode)
{
    ...
    if (osalHandler->MutexLock(s_commonMutex) != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("mutex lock error");
        gduReturnCode = GDU_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
        goto unlock2;
    }

    switch (mode) {
        case GDU_GIMBAL_RESET_MODE_YAW:
            ...
            break;
        case GDU_GIMBAL_RESET_MODE_PITCH_AND_YAW:
            ...
            s_attitudeHighPrecision.yaw = s_aircraftAttitude.yaw + s_systemState.fineTuneAngle.yaw;
            break;
        case GDU_GIMBAL_RESET_MODE_PITCH_DOWNWARD_UPWARD_AND_YAW:
            ...
            s_attitudeHighPrecision.yaw = s_aircraftAttitude.yaw + s_systemState.fineTuneAngle.yaw;
            break;
        case GDU_GIMBAL_RESET_MODE_PITCH_DOWNWARD_UPWARD:
            s_attitudeInformation.attitude.pitch =
                s_systemState.fineTuneAngle.pitch + (s_systemState.mountedUpward ? 900 : -900);
        ...
        default:
            ...
            gduReturnCode = GDU_ERROR_SYSTEM_MODULE_CODE_INVALID_PARAMETER;
            goto unlock1;
    }
...
{
    T_GduReturnCode gduReturnCode = GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS;
    T_GduGimbalReachLimitFlag attitudeReachLimitFlag = {0};
    T_GduGimbalReachLimitFlag fineTuneAngleReachLimitFlag = {0};
    T_GduAttitude3d aircraftAttitudeResetted = {0};
    T_GduAttitude3f attitudeFTemp = {0};
    T_GduOsalHandler *osalHandler = GduPlatform_GetOsalHandler();
    ...
    attitudeFTemp.roll = s_attitudeInformation.attitude.roll;
    attitudeFTemp.yaw = s_attitudeInformation.attitude.yaw;
    GduTest_GimbalAngleLegalization(&attitudeFTemp, s_aircraftAttitude, &attitudeReachLimitFlag);
    s_attitudeInformation.attitude.pitch = attitudeFTemp.pitch;
    s_attitudeInformation.attitude.roll = attitudeFTemp.roll;
    s_attitudeInformation.attitude.yaw = attitudeFTemp.yaw;
    ...
    GduTest_GimbalAngleLegalization(&attitudeFTemp, aircraftAttitudeResetted, &fineTuneAngleReachLimitFlag);
...
static T_GduReturnCode GduTest_GimbalAngleLegalization(T_GduAttitude3f *attitude, T_GduAttitude3d aircraftAttitude,
                                                       T_GduGimbalReachLimitFlag *reachLimitFlag)
{
    T_GduAttitude3d eulerAngleLimitMin;
    T_GduAttitude3d eulerAngleLimitMax;
    T_GduAttitude3d angleLimitInBodyCoordinateFromEulerLimitMin = {0};
    T_GduAttitude3d angleLimitInBodyCoordinateFromEulerLimitMax = {0};
    T_GduAttitude3d finalAngleLimitInBodyCoordinateMin = {0};
    T_GduAttitude3d finalAngleLimitInBodyCoordinateMax = {0};
    T_GduAttitude3f attitudeInBodyCoordinate = {0};
                             ...
                             T_GduAttitude3d *speed)
{
    ...
    GduTest_GimbalSpeedLegalization(speed);
                                                                           ...
                                                                           T_GduAttitude3d *attitude)
{
    ...
    attitude->pitch = aircraftPitchInRad * 180 / GDU_PI * 10;
    ...
    attitude->roll = aircraftRollInRad * 180 / GDU_PI * 10;
    ...
    attitude->yaw = aircraftYawInRad * 180 / GDU_PI * 10;
