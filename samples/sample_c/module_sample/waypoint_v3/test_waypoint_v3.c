...
T_GduReturnCode GduTest_WaypointV3RunSample(void)
{
    ...
    snprintf(tempPath, GDU_TEST_WAYPOINT_V3_KMZ_FILE_PATH_LEN_MAX, "%s/waypoint_file/waypoint_v3_test_file.xml",
             curFileDirPath);

    xmlFile = fopen(tempPath, "r");
    if (xmlFile == NULL) {
        USER_LOG_ERROR("Open xml file failed.");
        goto out;
    }
    ...
    returnCode = GduWaypointV3_UploadXmlFile(xmlFileBuf, xmlFileSize);
    if (returnCode != GDU_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
